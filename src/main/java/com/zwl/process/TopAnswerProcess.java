...
public class TopAnswerProcess extends PatternProcessor {
  ...
  public MatchOther processPage(Page page) {
    ...
    // 解析话题答案
    Map<Integer, CopyOnWriteArrayList<Answer>> map =
        object.getJSONArray("data").stream()
            .filter(
                o -> {
            ...
            .map(
                o -> {
                  ...
                  return new Answer()
                      ...
                      .setVoteupCount(voteupCount)
                      .setCommentCount(commentCount)
                      .setTitle(title);
                })
            .collect(
                Collectors.groupingBy(
                    Answer::getQuestionId, Collectors.toCollection(CopyOnWriteArrayList::new)));
  ...
  private void initAnswerFile(Long topicId) {
    ...
    // 初始化文件
    String fileName =
        ZhiHuConstant.PACKAGE_PATH
            + File.separator
            + "answer"
            + File.separator
            + "%s-%d.md".formatted(topic.getTopicName(), topic.getTopicId());
                ...
                buffer.append(
                    "- [%s的回答](%s),点赞数：%d，评论数：%d\n"
                        .formatted(
                            ...
                            StrUtil.format(
                                ZhiHuConstant.ANSWER_PAGE_URL,
                                answer.getQuestionId(),
                                answer.getAnswerId()),
                            answer.getVoteupCount(),
